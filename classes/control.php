...
function getOrgNr(){
	...
	$QUE = "select company_id from employer where u_id='" . $_SESSION['userid'] . "'";
        $res = $db->query($QUE) or die("Get Company : " . mysql_error());
        $row = mysqli_fetch_array($res);
        $companyId = $row['company_id'];
        if($companyId) {
            $nque = "SELECT company.orgnr FROM company WHERE company_id='$companyId'" ;
          $res = $db->query($nque) or die("Get orgNr : " . mysql_error());
          $row = mysqli_fetch_array($res);
          $orgnr = $row['orgnr'];
        }
        return $orgnr;
...
function getBusinessAgreement($orgnr){
        ...
        $res = $db->query($ba) or die("No ba status : " . mysql_error());
...
function getPaidStatus($orgnr){
	...
	$paid = "select paid from company where orgnr='$orgnr'";
        ...
        $res = $db->query($paid) or die("Get paid status : " . mysql_error());
	$rs = mysqli_fetch_array($res);
        $status = $rs['paid'];
