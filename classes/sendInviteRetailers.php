    ...
    function inviteRetailers() {
        ...
        switch ($mode) {
            ...
            case 'sendMailRetailers':
                ...
                break;
            case 'sendMailAdvtRetailers':
                $reseller = $_REQUEST['reseller'];
                $this->sendRetailersAdvtMail($reseller);
                break;
        }
    }
    ...
    function sendRetailersAdvtMail($reseller = '') {
        ...
        if ($advertiseId) {
            ...
            if ($reseller != '') {
                ...
                if ($value != '') {
                    ...
                    if (($checkCompanyId == '') && ($value != '')) {
                        ...
                        if (($emplResellerId == '') || ($emplResellerId == $_SESSION['userid'])) {
                            ...
                            $msg = BASE_URL . "viewResellerAdvertise.php?advertiseId=" . $advertiseId . '&ccode=' . $ccode . '&uId=' . $data['u_id'];
                        } else if ($emplResellerId != $_SESSION['userid']) {
                            $_SESSION['MESSAGE'] = NOT_ALLOWED_RETALER;
                            $url = BASE_URL . 'showResellerAdvertise.php';
                            $inoutObj->reDirect($url);
                ...
                $msg = BASE_URL . "viewAdvertiseRetailer.php?advertiseId=" . $advertiseId;
            }
        }
