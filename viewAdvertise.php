...
//echo "<pre>" ; print_r($data) ; echo  $lang ; echo  "<br>**" . $_GET['lang']  ; echo "</pre>";die();

$ownerName = $regObj->getOwnerName($data[0]['u_id']);
//echo "<ADVTpre>";print_r($ownerName);echo "</pre>";
if ($data[0] == '') {
    $inoutObj = new inOut();
...
?> </div>
                ...
                <table width="70%" BORDER=0 align="center" cellpadding="0" cellspacing="0">
                    <tr>
                        ...
                        <td width="50%" class="td_pad_right"><select style="width:250px; background-color:#e4e3dd; border:#abadb3 solid 1px" onChange="langChange(this.value,'<?= $_GET['advertiseId'] ?>','<?= $reseller ?>');" class="text_field_new" name="lang2" id="lang2" >
                        ...
                        foreach ($data['storeDetails'] as $data1['storeDetails']) {
                            ...
                            ?>
                                ...
                                <td align="center">MANUAL SWIPE</td>
                                        ...
                                        <? if ($data[0]['end_of_publishing'] > $current_date) { ?>
                                            <? if ($reseller == '') { ?>
                                        <td height="32" align="center"><a href="javascript:delete_advtStore('storeId=<?=$storeid ?>&advertiseId=<?= $data[0]['advertise_id'] ?>' )" onClick="" class="a2" title="Delete">
...
<script>
    function langChange(lang,advtId,reseller)
    {
        if(reseller!='')
        {
            javascript:location.href = "viewAdvertise.php?lang="+lang+"&advertiseId="+advtId+"&from="+reseller;
        }
        else
        {
            javascript:location.href = "viewAdvertise.php?lang="+lang+"&advertiseId="+advtId;
        }
    }
</script>
